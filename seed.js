...
function seed() {
  ...
  for (let i = 2; i < artists.length; i++) {
    ...
    // console.log(number)
    // console.log(typeof Math.floor(Math.random() * Math.floor(13)))
    const month_name = newDate.toString().slice(4, 7);

    let description = `Check out ${artistname} streaming to you live on ${month_name} ${newDate.getDate()}. Get your ticket now and come back to the event page at start time to tune in live!`
